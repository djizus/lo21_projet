...
Article& NotesManager::getArticle(const QString& fileName){
    // s'il existe deje, on le  renvoie
    for(unsigned int i=0; i<nbArticles; i++){
...
Document& NotesManager::getDocument(const QString& fileName){
    // on verifie d'abord que le document demande n'a pas deje ete charge
...
void NotesManager::saveArticle(Article& a){
    if (a.isModified()) {
        ...
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
            throw NotesException("Erreur sauvegarde d'un article : impossible d'ouvrir un fichier en ecriture");
...
void NotesManager::saveDocument(Document& d){
    if (d.isModified()) {
        ...
        // On ouvre notre fichier en lecture seule et on verifie l'ouverture
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
            throw NotesException("Erreur sauvegarde d'un document : impossible d'ouvrir un fichier en ecriture");
        QTextStream flux(&file);
        flux<<d;
        file.close();
        d.modified=false;
